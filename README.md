...
Orquesta la resolución completa de un issue de GitHub siguiendo un flujo estructurado en 5 fases, con pausa y aprobación del usuario entre cada una:

| Fase | Acción                                              |
|------|-----------------------------------------------------|
| 0    | Carga el issue con `gh issue view`                  |
| 1    | Delega análisis y plan al subagente `architect`     |
| 2    | Crea la rama y delega implementación a `general`    |
| 3    | Ejecuta `make lint` y `make test`                   |
| 4    | Hace el commit y abre el PR con `gh pr create`      |
...
No se usa directamente: `issue-flow` lo invoca en la Fase 1.
